...
void ScanForSlave() {
  ...
  if (SlaveCnt > 0) {
    ...
    tft.print(SlaveCnt); tft.println(" Slave(s) found, processing..");

  } else {
    Serial.println("No Slave Found, trying again.");
    tft.println("No Slave Found, trying again.");
  ...
  if (SlaveCnt > 0) {
    for (int i = 0; i < SlaveCnt; i++) {
      ...
      for (int ii = 0; ii < 6; ++ii ) {
        ...
        tft.print((uint8_t) slaves[i].peer_addr[ii], HEX);
        if (ii != 5) tft.print(":");
      }
      Serial.print(" Status: ");
      tft.print(" Status: ");
      ...
      if (exists) {
        ...
        if (addStatus == ESP_OK) {
          ...
          tft.println("Add Peer - Invalid Argument");
    ...
    tft.println("No Slave found to process");
  }
}
...
void setup() {
  ...
  tft.println ("COMPUTING");
  tft.println();
  tft.setTextColor(ST77XX_WHITE);
  tft.println ("  ESP-NOW protocol");
  tft.println();
  tft.println ("  ESP32 -> ESP8226");
  delay(3000);
  tft.fillScreen(ST77XX_BLACK);
